...
*Таблица до нормализации (1НФ, но не 2НФ):*

| СтудентID | КурсID | ИмяСтудента | НазваниеКурса | ИмяПреподавателя | Оценка |
|-----------|--------|-------------|---------------|------------------|--------|
| 1 | 101 | Иван | Математика | Петров | 5 |
| 1 | 102 | Иван | Физика | Сидоров | 4 |
| 2 | 101 | Мария | Математика | Петров | 5 |
| 2 | 103 | Мария | Химия | Иванов | 3 |

**Проблема:** 
- Первичный ключ: (СтудентID, КурсID)
- ИмяПреподавателя зависит только от КурсID (частичная зависимость)
- НазваниеКурса зависит только от КурсID (частичная зависимость)
- ИмяСтудента зависит только от СтудентID (частичная зависимость)
- Оценка зависит от всего ключа (СтудентID, КурсID) - это правильно

*Таблицы после нормализации (2НФ):*

**Таблица: Студент_Курс (связь многие-ко-многим)**
...
| КурсID | НазваниеКурса | ИмяПреподавателя |
|--------|---------------|------------------|
...
| СтудентID | ИмяСтудента |
|-----------|-------------|
| 1 | Иван |
| 2 | Мария |

**Результат:** Все неключевые атрибуты теперь полностью зависят от первичного ключа своей таблицы.
...
*Таблица до нормализации (2НФ, но не 3НФ):*

| СтудентID | ИмяСтудента | ГруппаID | НазваниеГруппы | Староста |
|-----------|-------------|----------|----------------|----------|
| 1 | Иван | 101 | ИТ-21 | Петров |
| 2 | Мария | 101 | ИТ-21 | Петров |
| 3 | Петр | 102 | ИТ-22 | Сидоров |
| 4 | Анна | 102 | ИТ-22 | Сидоров |

**Проблема:**
- Первичный ключ: СтудентID
- СтудентID -> ГруппаID (прямая зависимость от ключа)
- СтудентID -> НазваниеГруппы (через ГруппаID - транзитивная зависимость)
- СтудентID -> Староста (через ГруппаID - транзитивная зависимость)
- ГруппаID -> НазваниеГруппы (неключевой атрибут определяет другой)
- ГруппаID -> Староста (неключевой атрибут определяет другой)

*Таблицы после нормализации (3НФ):*

**Таблица: Студенты**

| СтудентID | ИмяСтудента | ГруппаID |
|-----------|-------------|----------|
| 1 | Иван | 101 |
| 2 | Мария | 101 |
| 3 | Петр | 102 |
...
| ГруппаID | НазваниеГруппы | Староста |
|----------|----------------|----------|
| 101 | ИТ-21 | Петров |
| 102 | ИТ-22 | Сидоров |

**Результат:** Транзитивные зависимости устранены. Все неключевые атрибуты зависят только от первичного ключа, а не от других неключевых атрибутов.
...
*   **Решение:** Вынести зависимость, где детерминант не является ключом, в отдельную таблицу.

**Пример преобразования в НФБК:**

*Таблица до нормализации (3НФ, но не НФБК):*

| СтудентID | КурсID | ПреподавательID | Оценка | ГруппаID |
|-----------|--------|-----------------|--------|----------|
| 1 | 101 | 201 | 5 | 10 |
| 1 | 102 | 202 | 4 | 10 |
| 2 | 101 | 201 | 5 | 11 |
...
**Проблема:**
- Первичный ключ: (СтудентID, КурсID)
- (СтудентID, КурсID) -> Оценка (правильно, зависит от всего ключа)
- (СтудентID, КурсID) -> ПреподавательID (правильно, зависит от всего ключа)
- СтудентID -> ГруппаID (детерминант СтудентID не является ключом, но определяет ГруппаID)
- Это нарушение НФБК, так как детерминант (СтудентID) не является потенциальным ключом

*Таблицы после нормализации (НФБК):*
...
**Результат:** Все детерминанты теперь являются потенциальными ключами (ключами-кандидатами).
...
**Пример преобразования в 4НФ:**

*Таблица до нормализации (НФБК, но не 4НФ):*

| СтудентID | КурсID | ХоббиID |
|-----------|--------|---------|
| 1 | 101 | 301 |
| 1 | 101 | 302 |
| 1 | 102 | 301 |
...
**Проблема:**
- Первичный ключ: (СтудентID, КурсID, ХоббиID)
- Многозначная зависимость: СтудентID ->-> КурсID (студент может изучать несколько курсов)
- Многозначная зависимость: СтудентID ->-> ХоббиID (студент может иметь несколько хобби)
- Курсы и хобби независимы друг от друга
- Это приводит к избыточности: если студент изучает 2 курса и имеет 2 хобби, нужно 2×2=4 строки

*Таблицы после нормализации (4НФ):*

**Таблица: Студент_Курс**
...
**Результат:** Многозначные зависимости устранены. Курсы и хобби хранятся в отдельных таблицах, что устраняет избыточность данных.

**4.2.6. Пятая нормальная форма (5НФ) и далее:**
*   **Требование:** 4НФ + нет зависимостей соединения (Join Dependencies).
*   **Зависимость соединения:** Таблица может быть восстановлена без потерь только путем соединения нескольких таблиц, но не из меньшего количества.
*   Борьба со сложными зависимостями соединения.
*   На практике редко используется, так как приводит к излишней декомпозиции.

**Пример преобразования в 5НФ:**

*Таблица до нормализации (4НФ, но не 5НФ):*

| ПоставщикID | ДетальID | ПроектID |
|-------------|----------|----------|
| S1 | D1 | P1 |
| S1 | D1 | P2 |
| S1 | D2 | P1 |
...
**Проблема:**
- Первичный ключ: (ПоставщикID, ДетальID, ПроектID)
- Зависимость соединения: таблица может быть восстановлена только соединением трех таблиц:
  - (ПоставщикID, ДетальID)
  - (ДетальID, ПроектID)
  - (ПроектID, ПоставщикID)
- Это означает, что информация о связях между поставщиками, деталями и проектами не может быть представлена меньшим количеством таблиц без потери информации

*Таблицы после нормализации (5НФ):*

**Таблица: Поставщик_Деталь**
...
**Результат:** Зависимости соединения устранены. Таблица разбита на три таблицы, которые можно соединить для восстановления исходной информации без потерь.
